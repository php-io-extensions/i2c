...
# Installer for the I2C extension on Debian Trixie (and compatible Ubuntu / Raspberry Pi OS).
...
header() {
    echo "=========================================="
    echo " I2C Extension Installer (Debian Trixie)"
    echo "=========================================="
    echo ""
}
...
# Ensure <linux/i2c-dev.h> is available (linux-libc-dev).
if ! dpkg -l linux-libc-dev >/dev/null 2>&1 || \
   ! [ -f /usr/include/linux/i2c-dev.h ]; then
...
# then locate the co-installed php-config for the correct extension dir.
